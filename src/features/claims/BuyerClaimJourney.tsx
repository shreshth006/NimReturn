      ...
      {!claim && (
        <form className="claim-form" onSubmit={(event) => void create(event)}>
          <div className="claim-type-picker" role="group" aria-label="Claim type">
            <button className={claimType === 'RETURN' ? 'selected' : ''} type="button" disabled={!returnAvailable} onClick={() => setClaimType('RETURN')}><strong>Return</strong><span>{passport.deadlines.return ? `Through ${new Date(passport.deadlines.return).toLocaleDateString()}` : 'Not offered'}</span></button>
            <button className={claimType === 'WARRANTY' ? 'selected' : ''} type="button" disabled={!warrantyAvailable} onClick={() => setClaimType('WARRANTY')}><strong>Warranty</strong><span>{passport.deadlines.warranty ? `Through ${new Date(passport.deadlines.warranty).toLocaleDateString()}` : 'Not offered'}</span></button>
          </div>
