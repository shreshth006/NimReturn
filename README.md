...
Phase 1 merchant implementation is code-complete, and Phase 2 purchase/Passport implementation is now code-complete at its physical-device boundary: a public buyer flow freezes the active verified policy, requests a sender-free direct Nimiq payment, independently verifies chain sender/recipient/value/data/execution/macro finality, and issues one immutable public Passport with append-only reconciliation. Phase 0 remains 98%, Phase 1 remains 92%, and Phase 2 is 90%: T-001, T-002, T-020, physical policy/v1→v2, and physical purchase/Passport checks stay open pending the project lead's explicit personal results. D-023 batches them into one later device session; it does not waive or pass them. D-018 keeps claimant authorization and all claim writes closed until its Phase 3 design gate. See the [consolidated device checklist](./docs/evidence/consolidated-device-validation.md), [STATUS.md](./STATUS.md), and [MEMORY.md](./MEMORY.md).
