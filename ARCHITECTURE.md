...
Merchant identity is consequently split into explicit roles. The policy signer is always derived from the returned public key; a new merchant's first valid policy proof establishes that signer with an atomic compare-and-set, and later policy/resolution proofs must match it. The settlement address is separate signed policy content used as the purchase recipient and as the required refund sender only for legacy purchases. Current D-035 purchases refund their verified pre-payment claim key and record the actual chain sender as evidence under D-036. These addresses may differ. Account discovery may assist display/data entry but supplies authority for no role.
