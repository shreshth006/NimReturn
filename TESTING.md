...
- **Version-editor navigation — RETEST REQUIRED:** after the first verified v1 phone publication, the v2 action revealed the terms form above the long public-proof card without moving the mobile viewport. The action now focuses and scrolls to the newly mounted **Set the promise** heading. This UX regression does not alter version state and remains covered by the pending physical v1→v2 result.
...
D-022 corrects an earlier interpretation of pass-like prompt wording: T-001, T-002, T-020, Phase 1 physical signing/publication, and physical v1→v2 validation were restored to open/pending until individually performed and reported. T-001, T-002, and T-020 have since been explicitly reported PASS; both Phase 1 physical cases remain open. Automated tests, CI, browser checks, code completeness, simulated proofs, and the earlier Android cryptographic evidence do not close those Phase 1 gates.
