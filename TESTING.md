...
- **Version-editor navigation — addressed:** after the first verified v1 phone publication, the v2 action revealed the terms form above the long public-proof card without moving the mobile viewport. The action now focuses and scrolls to the newly mounted **Set the promise** heading. The later physical v1→v2 result passed.
...
- **T-024 Wrong network:** every wallet action first requires provider consensus and a bounded head match against the API's configured verifier network; server verification still rejects any exact network mismatch. The head comparison is a fail-closed heuristic because the wallet SDK exposes no discriminating network ID.
...
- **T-040 Claim authorization:** accepted authority is exactly one of chain-sender equality, a D-035 key proven over the unpaid order before payment, or an exact-claim authorization proof derived to the purchase sender. Cover every accepted path plus missing/unrelated/altered/replayed/expired authorization rejection. No test may treat a client-selected account as proof.
...
- **T-062 Wrong sender:** legacy purchases reject any sender other than the purchase-bound signed settlement address; D-035 purchases record the actual chain sender and do not mistake it for merchant identity.
- **T-063 Wrong recipient:** D-035 purchases require the pre-payment claim key; legacy purchases require the original chain buyer.
...
Phase 3 self-authorized claim — **resolved and physical retest PASS:** the first Android attempt exposed a strict proof-envelope client bug and an expired unsigned-claim recovery gap. The envelope builder and migration 0013 fixed those failures; the project lead later reported the physical claim result PASS.

Phase 3 legacy delegated claim — **wallet limitation preserved:** the wallet could not select the chain-sender key, so the proof correctly failed. D-035 added a purchase claim key for new purchases without weakening the legacy exact-claim delegation rule.

Phase 3 wallet account routing — **observed, design changed, and physical retest PASS (D-035):** payments came from one wallet construct while signatures used its owner key and no account selection was available. A fresh purchase bound that signing key before payment, and the resulting distinct-signer claim passed physically on 2026-09-17.

Phase 4 refund routing — **observed, design changed, and physical retest PASS (D-036):** the first request returned no hash and nothing reached the chain; Nimiq Pay's contract sender made the settlement-sender rule unusable. A fresh claim-key refund later passed independent chain verification and macro finality. PostgreSQL tests cover wrong recipient, amount, and tag, reused hashes, forged recipients, early or evidence-backed rule-outs, incomplete history, RPC failure, and recovery.
...
The Phase 2 automated suite passes T-021 through T-029 and the implementation portions of T-031 through T-033, including a simulated finalized-evidence regression. Device payment, chain verification, Passport, reload, and mobile results passed; only the deferred first-minute result remains open. The exact consolidated procedure is `docs/evidence/consolidated-device-validation.md`.

The Phase 3 automated suite passes the implementation portions of T-040 through T-055 across chain-sender, purchase-key, and delegated claimant authority; strict proof failures; inclusive eligibility boundaries; purchase-bound historical policy; atomic outage behavior; protected merchant review; policy-signer-only decisions; idempotent/concurrent finality; and approved-versus-paid copy. Physical self/distinct-signer claims and resolution passed; reload and mobile/accessibility remain open.

The Phase 4 automated suite covers immutable server-derived refund expectations; strict merchant ownership; cancellation, unknown-outcome, chain-history recovery/rule-out, pending, failure, and finalized states; D-036 sender/recipient rules; exact value/data/network/execution/finality checks; global hash replay rejection; idempotent and concurrent verification; the atomic Passport-refunded transition; immutable transaction evidence; append-only reconciliation; and strict frontend/session response parsing. Physical real-chain finality passed; native cancellation, WebView recovery, and mobile/accessibility remain open.

The Phase 5 automated suite passes T-071 and T-072 and the automated portions of T-073/T-074. A mixed PostgreSQL fixture reconciles 3 verified purchases; 4 filed claims split into 3 eligible/1 ineligible; 2 approved/1 rejected/1 unresolved; 1 refund pending/1 verified refund; and a three-resolution timing sample. Runtime-role writes fail, no mutation route exists, and reconciliation exceptions remain append-only. Coverage also proves that the featured example must freshly reverify as refunded, the Passport claim list exposes only a strict minimal projection, and wallet prompts stop before signing/payment when consensus or the verifier-network head cannot be matched. T-080 through T-084 retain manual/device portions.
