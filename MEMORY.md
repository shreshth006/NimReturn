...
Phase 0 remains 98%, Phase 1 remains 92%, and Phases 2–5 remain 90% experimental/code-complete at their external-validation boundaries. The project lead explicitly reported T-001 provider failure/recovery and T-002 account-permission cancellation/recovery PASS on 2026-09-16; all other named physical wallet, policy, purchase, Passport, claim, decision, refund, reload, mobile/accessibility, and first-minute results remain open/pending until separately performed and reported. D-030 records the project lead's explicit Phase 5 instruction and the read-only verified-evidence Promise Ledger. D-025 claimant authorization and D-028 exact-sender refunds remain fail-closed, but actual Nimiq Pay account-routing usability is unproven. Phase 6 is not started or authorized.
...
Physical Android Nimiq Pay has passed T-001 provider failure/recovery and T-002 account-permission cancellation/recovery. It must still prove T-020, merchant signing/v1→v2, purchase/Passport, claim authorization, merchant resolution, an exact-sender refund through finality, reload, mobile/accessibility behavior, Promise Ledger reconciliation, and the first-minute story. Distinct-claim-signer authorization and settlement-sender routing are the highest-risk checks. iOS is explicitly deferred by D-016, not claimed compatible. Temporary HTTPS staging is live at `https://nimreturn-staging-cycle2.onrender.com` with a free managed PostgreSQL database (expires 2026-10-16), generated session secret, least-privilege runtime login, and passing Phase-5/TestAlbatross preflight. Managed backup/restore proof, alerting/runbooks, an operated primary/failover RPC, and actual Nimiq Pay CSP compatibility are absent. Multi-instance rate limiting is not supported. The public development RPC is not production-grade.

# Next actions

1. Open `https://nimreturn-staging-cycle2.onrender.com` in Nimiq Pay and run the consolidated physical Phase 0–5 and judge-surface checklist; report an inability to route the purchase-bound settlement account as FAIL rather than weakening sender verification.
